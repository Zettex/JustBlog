...
namespace JustBlog.Core
{
    public class BlogRepository : IBlogRepository
    {
        ...
        public IList<Post> PostsForFeed(int count)
        {
            var posts = _session.Query<Post>()
                                  .Where(p => p.Published)
                                  .OrderByDescending(p => p.PostedOn)
        ...
        public User User(int userId) => _session.Query<User>().FirstOrDefault(u => u.Id == userId);

        public User User(string username) => _session.Query<User>().FirstOrDefault(u => u.Nickname.Contains(username));

        public string Login(string usename, string password)
        {
            var user = _session.Query<User>().FirstOrDefault(u => u.Nickname == usename && u.Password == password);
        ...
        public bool UserNameExist(string username) => _session.Query<User>().FirstOrDefault(u => u.Nickname == username) != null;

        public bool UserEmailExist(string email) => _session.Query<User>().FirstOrDefault(u => u.Email == email) != null;
        ...
        {
            var user = _session.Query<User>().FirstOrDefault(u => u.Nickname.Equals(username));
